...
def parse_nmap(scan_dir="scan", output=None, probe_tls=False, jump_host=None):
    ...
    output_path = output or os.path.join(scan_dir, "parsed.json")
    with open(output_path, "w") as f:
        json.dump(parsed_data, f, indent=2)
    print(f"[+] Wrote: {output_path}")

    _generate_cmds(scan_dir, all_services, services_index)
    _print_summary(parsed_data)

    return parsed_data


def _generate_cmds(scan_dir, services, services_index):
    tool_files = {}
    ...
    for tool_name, cfg in TOOL_CMDS.items():
        ...
        outdir = f"{scan_dir}/run-output/{tool_name}"
        ...
        if lines:
            cmds_file = os.path.join(scan_dir, f"{tool_name}.cmds")
    ...
    _generate_main_sh(scan_dir, tool_files)


def _generate_main_sh(scan_dir, tool_files):
    path = os.path.join(scan_dir, "main.sh")
    with open(path, "w") as f:
        ...
        for tool_name in sorted(tool_files):
            f.write(f'mkdir -p {scan_dir}/run-output/{tool_name}\n')
        f.write('\n')
